...
function Mammoapp() {
	...
	function initialize() {
		...
		WS.orientMag = cWidth/(0.2*document.width);
		...
		//theCanvas_obox1.width = Math.round(WS.canvasWidth / WS.orientMag);
		//theCanvas_obox1.height = Math.round(WS.canvasHeight / WS.orientMag);
		//theCanvas_obox2.width = Math.round(WS.canvasWidth / WS.orientMag);
		//theCanvas_obox2.height = Math.round(WS.canvasHeight / WS.orientMag);
		...
		WS.viewCM = false;
		APP.maxRoiArea = (0.25*cWidth*cHeight);
...
Mammoapp.prototype.constructImpressionsForm = function() {
	...
	if (APP.findings.length > APP.answerfindings.length) {
		var dif = (APP.findings.length - APP.answerfindings.length);
		str += "<br>"+dif+" incorrect ROI's (<b>-"+(dif*10)+" points</b>)";
		score = score - 10*dif;
	}
	...
	if (APP.caseSubmitted) {
		...
		scorebox.innerHTML = "Score: "+ score +"/40";
		if(score >=30){
