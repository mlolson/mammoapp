...
function canvasApp() {
	if (!canvasSupport()) {
		...
		imgC.src = window.STATIC_URL + "media/set" + setNum + "/" + imageNum
				+ "c.jpg";
		imgM.src = window.STATIC_URL + "media/set" + setNum + "/" + imageNum
				+ "m.jpg";
...
function initialize() {
	...
	while (theCanvas_obox1.width != Math.round(cWidth / orientMag)
			|| theCanvas_obox2.width != Math.round(mWidth / orientMag)) {
	...
	if (theCanvas_top.addEventListener) {
		...
		// Firefox
		theCanvas_top.addEventListener("DOMMouseScroll", MouseWheelHandler,
				false);
	}
	// IE 6/7/8
	else
...
function drawScreen() {
	...
	if (isRectRoiDrawing) {
		drawScreen_top_only();
			...
			&& findings[selectedFinding].wasJustCompleted) {
		...
		if (viewCM) {
			context_bottom.drawImage(imgM, 0, 0, windowWidth, windowHeight,
					windowX, windowY, windowWidth * zoomFactor, windowHeight
							* zoomFactor);
		} else {
			context_bottom.drawImage(imgC, 0, 0, windowWidth, windowHeight,
					windowX, windowY, windowWidth * zoomFactor, windowHeight
							* zoomFactor);
		}
		// console.log("zoom = " +zoomFactor +", x = " + windowX + ", y =
		// "+windowY);
		// context.font = "bold 18px sans-serif";
...
function drawScreen_top_only() {
	...
	for (i = 0; i < answerfindings.length; i++) {
		if ((answerfindings[i].isCC && !viewCM)) {
			...
			context_top.lineTo(((xx) * zoomFactor) + windowX - 10, (yy)
					* zoomFactor + windowY - 10);
			context_top.lineTo(((xx) * zoomFactor) + windowX + 10, (yy)
					* zoomFactor + windowY + 10);
			context_top.stroke();
			...
			context_top.beginPath();
			context_top.lineTo(((xx) * zoomFactor) + windowX - 10,
					((yy) * zoomFactor) + windowY + 10);
			context_top.lineTo(((xx) * zoomFactor) + windowX + 10,
					((yy) * zoomFactor) + windowY - 10);
			context_top.stroke();
			context_top.closePath();

			context_top.fillText("F" + (answerfindings[i].findingNum + 1),
					(xx * zoomFactor) + windowX, (yy * zoomFactor) + windowY);
			context_top.stroke();
			context_top.closePath();
...
function drawOBox() {
	...
	context_obox1.drawImage(imgC, 0, 0, iWidth, iHeight, 0, 0, (iWidth)
			/ orientMag, (iHeight) / orientMag);

	iHeight = imgM.height;
	iWidth = imgM.width;

	context_obox2.clearRect(0, 0, canvasWidth, canvasHeight);
	context_obox2.drawImage(imgM, 0, 0, iWidth, iHeight, 0, 0, (iWidth)
			/ orientMag, (iHeight) / orientMag);

	if (!viewCM) {
		context_obox1.strokeStyle = '#f00'; // red
		context_obox1.strokeRect(0 - ((windowX / zoomFactor) / orientMag),
				0 - ((windowY / zoomFactor) / orientMag),
				(((canvasWidth / zoomFactor))) / orientMag,
				(((canvasHeight / zoomFactor))) / orientMag);
		context_obox1.font = "10px sans-serif";
	} else {
		context_obox2.strokeStyle = '#f00'; // red
		context_obox2.strokeRect(0 - ((windowX / zoomFactor) / orientMag),
				0 - ((windowY / zoomFactor) / orientMag),
				(((canvasWidth / zoomFactor))) / orientMag,
				(((canvasHeight / zoomFactor))) / orientMag);
	...
	for (i = 0; i < answerfindings.length; i++) {
		if (answerfindings[i].isCC) {
			...
			context_obox1.fillText("F" + (answerfindings[i].findingNum + 1),
					((answerfindings[i].xCC) / orientMag),
					0 + ((answerfindings[i].yCC) / orientMag));
		}
		if (answerfindings[i].isMLO) {
			...
			context_obox2.fillText("F" + (answerfindings[i].findingNum + 1),
					((answerfindings[i].xMLO) / orientMag),
					0 + ((answerfindings[i].yMLO) / orientMag));
...
function drawSpotMag(x, y) {
	context_top.clearRect(0, 0, canvasWidth, canvasHeight);
	var mag = 1.2;
	var xx = ((x - 100) / zoomFactor) - windowX / zoomFactor - 100
			/ (zoomFactor + mag);
	var yy = ((y - 100) / zoomFactor) - windowY / zoomFactor - 100
			/ (zoomFactor + mag);
	if (viewCM) {
		context_top.drawImage(imgM, xx, yy, 200 / (zoomFactor + mag),
				200 / (zoomFactor + mag), x - 200, y - 200, 200, 200);
	} else {
		context_top.drawImage(imgC, xx, yy, 200 / (zoomFactor + mag),
				200 / (zoomFactor + mag), x - 200, y - 200, 200, 200);
	}
}
...
function MouseDownHandler(e) {
	...
	if (this.id == "canvas_obox1") {
		...
		// windowX = (canvasWidth-(zoomFactor*windowWidth)) +
		// (((windowWidth/orientMag-(e.layerX)))*zoomFactor*orientMag) -
		// canvasWidth/2;
		// windowY = (canvasHeight-(zoomFactor*windowHeight)) +
		// ((windowHeight/orientMag- (e.layerY))*zoomFactor*orientMag) -
		// canvasHeight/2;
		windowX = (canvasWidth - (zoomFactor * windowWidth))
				+ (((windowWidth / orientMag - (e.layerX + 80))) * zoomFactor * orientMag)
				- canvasWidth / 2;
		windowY = (canvasHeight - (zoomFactor * windowHeight))
				+ ((windowHeight / orientMag - (e.layerY + 2)) * zoomFactor * orientMag)
				- canvasHeight / 2;
	...
	if (this.id == "canvas_obox2") {
		...
		windowX = (canvasWidth - (zoomFactor * windowWidth))
				...
				- canvasWidth / 2;
		windowY = (canvasHeight - (zoomFactor * windowHeight))
				+ ((windowHeight / orientMag - (e.layerY + 29)) * zoomFactor * orientMag)
				- canvasHeight / 2;
		dragCoordStart = [ e.screenX, e.screenY ];
		drawScreen();
		return;
	...
	/*
	 * if(isInOrientationBox(e.layerX,e.layerY)){ //needs refactoring isDragging =
	 * true; windowX = (canvasWidth-(zoomFactor*windowWidth)) +
	 * (((windowWidth/orientMag-(e.layerX-orientX)))*zoomFactor*orientMag) -
	 * canvasWidth/2; windowY = (canvasHeight-(zoomFactor*windowHeight)) +
	 * ((windowHeight/orientMag- e.layerY)*zoomFactor*orientMag) -
	 * canvasHeight/2; dragCoordStart = [e.screenX,e.screenY]; drawScreen();
	 * return; }
	 */
...
function MouseMoveHandler(e) {
	if (isSpotMagTool) {
		...
		if (this.id == "canvas_obox1" || this.id == "canvas_obox2") {
			windowX = windowX - orientMag * (x - dragCoordStart[0])
					* zoomFactor;
			windowY = windowY - orientMag * (y - dragCoordStart[1])
					* zoomFactor;
		} else {
...
function isFindingClicked(x, y) {
	...
	for (i = 0; i < answerfindings.length; i++) {
		if (answerfindings[i].isCC && !viewCM) {
			...
			if (xx > xo && yy > yo && xx < xo + (20 / zoomFactor)
					&& yy < yo + (20 / zoomFactor)) {
				return i;
			}
		} else if (answerfindings[i].isMLO && viewCM) {
...
function gotoNextFinding() {
	if (answerfindings.length > 0) {
		selectedFinding++;
		if (selectedFinding > answerfindings.length - 1) {
			selectedFinding = 0;
		}
		drawScreen();
		findingDropdownChangeHandler();
...
function constructImpressionsForm() {
	...
	for (i = 0; i < answerfindings.length; i++) {
		...
		newDiv.innerHTML = "Finding " + (answerfindings[i].findingNum + 1)
				+ "<br>" + "Type: " + answerfindings[i].type + "<br>"
...
function findingDropdownChangeHandler() {
	...
	// }
	// console.log(s);
	document.getElementById('finding_indicator').innerHTML = "Finding: "
			+ (selectedFinding + 1) + "/" + answerfindings.length;
	...
	function addPoint(x, y) {
		...
		if (this.n > 2) {
			if ((Math.abs(this.xArr[0] - this.xArr[this.n]) + Math
					.abs(this.yArr[0] - this.yArr[this.n])) < 100) {
		...
		this.n++;
		// console.log("n, x, y =
		// "+this.n+","+this.xArr[this.n]+","+this.yArr[this.n]);
	}
...
function saveAnswerFinding() {
	answerfindings[selectedFinding].description = document
			.getElementById("description_box").value;
	// console.log("description_box value= "+
	...
	if (calc) {
		answerfindings[selectedFinding].type = "calc";
	} else if (mass) {
		answerfindings[selectedFinding].type = "mass";
	} else if (other) {
		answerfindings[selectedFinding].type = "other";
	}
...
function saveFinding() {
	...
	findings[selectedFinding].description = document
			.getElementById("description_box").value;
	// console.log("description_box value= "+
	// document.getElementById("description_box").value);
	var mass = check1.checked;
	...
	if (calc) {
		findings[selectedFinding].type = "calc";
		console.log("calc saved");
	} else if (mass) {
		findings[selectedFinding].type = "mass";
	} else if (other) {
		findings[selectedFinding].type = "other";
	}
...
function deleteFinding() {
	var dropdown = document.getElementById("findingsDrop");
	Dajaxice.finding.deleteFinding(callback_deleteFindingForm, {
		'numToDelete' : (dropdown.options[dropdown.selectedIndex].text)
	});
	// getFindings();
}
