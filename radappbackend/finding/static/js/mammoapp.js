 ...
 *                Beta version. Works best with Google Chrome.
 *                Results not guaranteed in other browsers.
 *                This version is under development: some code is obsolete, 
 *                I have tried to mark it as such where it has been left in.
 *                
...
function Mammoapp(){
	...
	function initialize(){
		...
		if (theCanvas_top.addEventListener) {  
			...
			theCanvas_top.addEventListener("mousewheel", this.MouseWheelHandler, false); 
	    	theCanvas_top.addEventListener("mousedown", this.MouseDownHandler, false); 
	    	theCanvas_top.addEventListener("mousemove", this.MouseMoveHandler, false);
	    	theCanvas_top.addEventListener("mouseup", this.MouseUpHandler, false);
	    	theCanvas_top.addEventListener("mouseout", this.MouseLeaveHandler, false);
	    	//canvas.addEventListener("mouseout", MouseUpHandler, false);
	    	// Firefox  
	    	theCanvas_top.addEventListener("DOMMouseScroll", this.MouseWheelHandler, false);  
	    }  
	    if (theCanvas_obox1.addEventListener) {
	        theCanvas_obox1.addEventListener("mousedown", this.MouseDownHandler, false); 
	    	theCanvas_obox1.addEventListener("mousemove", this.MouseMoveHandler, false);
	    	theCanvas_obox1.addEventListener("mouseup", this.MouseUpHandler, false);
	    	theCanvas_obox1.addEventListener("mouseout", this.MouseLeaveHandler, false);
	    }
	    if (theCanvas_obox2.addEventListener) {
	        theCanvas_obox2.addEventListener("mousedown", this.MouseDownHandler, false); 
	    	theCanvas_obox2.addEventListener("mousemove", this.MouseMoveHandler, false);
	    	theCanvas_obox2.addEventListener("mouseup", this.MouseUpHandler, false);
	    	theCanvas_obox2.addEventListener("mouseout", this.MouseLeaveHandler, false); 	
	    }
		...
		Dajaxice.finding.getMaxImageNum(callback_setMaxImgNum,{"setNum":APP.setNum})
		Dajaxice.finding.returnAllAnswerFindings(callback_checkFindingAnswers,{"setNum":APP.setNum,"imageNum":APP.imageNum});		
		APP.drawScreen(); 
	}
	...
	if(!Modernizr.canvas) {
		...
		this.setNum = 1;
		this.imageNum =1;
		this.imgC.src = window.STATIC_URL+"media/set"+this.setNum+"/"+this.imageNum+"c.jpg";
		this.imgM.src = window.STATIC_URL+"media/set"+this.setNum+"/"+this.imageNum+"m.jpg";
...
/********************************************************************************************************
 * 
 *		Canvas Event Handler Functions : Fired on top canvas element and orientation boxes
 * 
 ********************************************************************************************************/
...
Mammoapp.prototype.MouseDownHandler = function(e){
	if(this.id == "canvas_obox1"){
		...
		WS.windowX = (WS.canvasWidth-(WS.zoomFactor*WS.windowWidth)) + 
							(((WS.windowWidth/WS.orientMag-(e.layerX)))*WS.zoomFactor*WS.orientMag) - 
							WS.canvasWidth/2;
		WS.windowY = (WS.canvasHeight-(WS.zoomFactor*WS.windowHeight)) + 
						((WS.windowHeight/WS.orientMag- (e.layerY))*WS.zoomFactor*WS.orientMag) - 
						WS.canvasHeight/2;
...
Mammoapp.prototype.MouseMoveHandler = function(e){
	...
	if(WS.isDragging){	
		...
		if(this.id =="canvas_obox1" || this.id == "canvas_obox2"){
			WS.windowX = WS.windowX - WS.orientMag*(x -WS.dragCoordStart[0])*WS.zoomFactor;
			WS.windowY = WS.windowY - WS.orientMag*(y -WS.dragCoordStart[1])*WS.zoomFactor;			
		}else{
...
Mammoapp.prototype.isAnswerFindingClicked = function(x,y){
	...
	for(var i=0;i<APP.answerfindings.length;i++){
		...
		if((!WS.viewCM && afind.isCC) || (WS.viewCM && afind.isMLO)){
			...
			if(xx >xo-rng && yy>yo-rng && xx< xo+rng && yy< yo+rng){
				return i;
			}
		}	
	}
...
Mammoapp.prototype.drawScreen = function() {
    ...
    if(WS.isRectRoiDrawing){ //Only draw top screen if we are in draw ROI mode
    	APP.drawScreen_top_only();
    }else if(APP.findings.length>0 && APP.findings[APP.selectedFinding].wasJustCompleted){ 
    	//Work around for bug where ctx stalls while drawing img and shapes... I don't fully understand why.
		...
		if(WS.viewCM){
			APP.context_bottom.drawImage(APP.imgM, 0,0 , WS.windowWidth, WS.windowHeight,
					WS.windowX, WS.windowY,WS.windowWidth*WS.zoomFactor,
					WS.windowHeight*WS.zoomFactor);
		}else{
			APP.context_bottom.drawImage(APP.imgC, 0,0 , WS.windowWidth, WS.windowHeight,
					WS.windowX, WS.windowY,WS.windowWidth*WS.zoomFactor,
					WS.windowHeight*WS.zoomFactor);
		}
    	APP.drawScreen_top_only();   	
...
Mammoapp.prototype.drawScreen_top_only = function(){
	...
	APP.context_top.clearRect (0, 0, WS.canvasWidth, WS.canvasHeight);
	//these two properties can probably be set at initialize since they do not change.
	...
	var ypts;
	var npts;
	//Don't know how I feel about this little feature. It might be not particularly useful
	//because mammos typically have 1 or two findings tops.
	APP.context_top.fillStyle = "white";
	if(WS.isPolyRoiDrawing){
		APP.context_top.fillText("Click to define points of polygon ROI...",230,20);
	}else{
		APP.context_top.fillText((APP.answerfindings.length - APP.numFoundAnswerFindings)+ " findings left",10,10);
	...
	for(i=0;i<APP.findings.length;i++){
		...
		if(find.view==WS.viewCM){
			...
			for(j=0;j<npts;j++){
				...
				APP.context_top.lineTo((xpts[j]*WS.zoomFactor)+WS.windowX, (ypts[j]*WS.zoomFactor)+WS.windowY);
			} 
			if(!find.isClosed){
				APP.context_top.strokeStyle = "white";
				APP.context_top.fillStyle = "white";
				//APP.context_top.arc((xpts[0]*WS.zoomFactor)+WS.windowX, (ypts[0]*WS.zoomFactor)+WS.windowY, 20/WS.zoomFactor, 0 , 2 * Math.PI, false);
				APP.context_top.lineTo(WS.currentMousePos[0],WS.currentMousePos[1]);
				
				//APP.context_top.closePath();
				APP.context_top.stroke();
				//Draw circle at origin
				APP.context_top.beginPath();
				APP.context_top.arc((xpts[0]*WS.zoomFactor)+WS.windowX, (ypts[0]*WS.zoomFactor)+WS.windowY, 15, 0 , 2 * Math.PI, false);
				
				APP.context_top.closePath();
				APP.context_top.stroke();
			}else{	
				//APP.context_top.fillText("F"+(find.findingNum+1),(xpts[0]*WS.zoomFactor)+WS.windowX, (ypts[0]*WS.zoomFactor)+WS.windowY);
				APP.context_top.closePath();
				APP.context_top.stroke();
			}
			//APP.context_top.stroke();
...
Mammoapp.prototype.drawOBox = function(){
	...
	APP.context_obox1.drawImage(APP.imgC, 0,0 , iWidth, iHeight,0, 0,(iWidth)/WS.orientMag,(iHeight)/WS.orientMag);
	
	iHeight = APP.imgM.height;
	iWidth = APP.imgM.width;
	
	APP.context_obox2.clearRect (0, 0, WS.canvasWidth, WS.canvasHeight);	
	APP.context_obox2.drawImage(APP.imgM, 0,0 , iWidth, iHeight,0, 0,(iWidth)/WS.orientMag,(iHeight)/WS.orientMag);	
	
	if(!WS.viewCM){
		APP.context_obox1.strokeStyle = '#f00'; // red
		APP.context_obox1.strokeRect(0-((WS.windowX/WS.zoomFactor)/WS.orientMag),0-((WS.windowY/WS.zoomFactor)/WS.orientMag),
				(((WS.canvasWidth/WS.zoomFactor)))/WS.orientMag,(((WS.canvasHeight/WS.zoomFactor)))/WS.orientMag);
		APP.context_obox1.font = "10px sans-serif";
	}else{
		APP.context_obox2.strokeStyle = '#f00'; // red
		APP.context_obox2.strokeRect(0-((WS.windowX/WS.zoomFactor)/WS.orientMag),0-((WS.windowY/WS.zoomFactor)/WS.orientMag),
				(((WS.canvasWidth/WS.zoomFactor)))/WS.orientMag,(((WS.canvasHeight/WS.zoomFactor)))/WS.orientMag);
	...
	for(var i=0;i<APP.findings.length;i++){
		var xpts = APP.findings[i].xArr;
 		...
 		if(!APP.findings[i].isClosed){
			...
			for(var j=0;j<afnums.length;j++){
				...
				if(af.wasSubmitted){
					...
					APP.context_obox1.strokeStyle = '#FFCC00'; //Darker Orange-yellow
					APP.context_obox2.strokeStyle = '#FFCC00'; 
				}
				if(af.isCC){
					APP.context_obox1.beginPath();
					APP.context_obox1.lineTo((af.xCC/WS.orientMag) - 5, (af.yCC/WS.orientMag) -5);
					APP.context_obox1.lineTo((af.xCC/WS.orientMag) + 5, (af.yCC/WS.orientMag) +5);
				...
				if(af.isMLO){
					...
					APP.context_obox2.lineTo((af.xMLO/WS.orientMag) - 5, (af.yMLO/WS.orientMag) -5);
					APP.context_obox2.lineTo((af.xMLO/WS.orientMag) + 5, (af.yMLO/WS.orientMag) +5);
	 	...
	 	/*if(APP.findings[i].findingNum == APP.selectedFinding){
	 		...
	 		APP.context_obox1.lineWidth = 1;
	 		APP.context_obox2.lineWidth = 1;
	 	}*/
		if(!APP.findings[i].view){
		 	APP.context_obox1.beginPath();
		 	for(var j=0;j<npts;j++){
				APP.context_obox1.lineTo(xpts[j]/WS.orientMag,ypts[j]/WS.orientMag);
			} 
		 	//APP.context_obox1.closePath();
		 	APP.context_obox1.stroke();
		}else{
		 	APP.context_obox2.beginPath();
			for(var j=0;j<npts;j++){
				APP.context_obox2.lineTo(xpts[j]/WS.orientMag,ypts[j]/WS.orientMag);
			}
			//APP.context_obox2.closePath();
		 	APP.context_obox2.stroke();
		}
	...
	if(WS.DisplayAnswers){
		for(var i=0;i<APP.answerfindings.length;i++){
			...
			if(afind.isCC){
				...
				APP.context_obox1.fillText("A"+(afind.findingNum+1),afind.xCC/WS.orientMag,afind.yCC/WS.orientMag);					
			}
			if(afind.isMLO){
				if(afind.corFinding==-1){
				...
				APP.context_obox2.fillText("A"+(afind.findingNum+1),afind.xMLO/WS.orientMag,afind.yMLO/WS.orientMag);								
			}		
		}	
	}
}
//Draw X where the answer finding is. Make it green if it is "correct", else red.
Mammoapp.prototype.drawAnswerFindingMark = function(n){
	...
	var stk, lw, fs; 
	if(APP.isAnswerFindingClicked(WS.currentMousePos[0],WS.currentMousePos[1])==af.findingNum){
		lw = 5;
	...
	//APP.context_top.fillText("A"+(af.findingNum+1),(xx*WS.zoomFactor)+WS.windowX, (yy*WS.zoomFactor)+WS.windowY);
	APP.context_top.fillText(lbl,(xx*WS.zoomFactor)+WS.windowX+10, (yy*WS.zoomFactor)+WS.windowY-7);
	APP.context_top.stroke();
	APP.context_top.closePath();
}
Mammoapp.prototype.drawSpotMag = function(x,y){
	APP.context_top.clearRect (0, 0, WS.canvasWidth, WS.canvasHeight);
	var mag = 1.2;
	var xx =  ((x-100)/WS.zoomFactor)-WS.windowX/WS.zoomFactor - 100/(WS.zoomFactor+mag);
	var yy =  ((y-100)/WS.zoomFactor)-WS.windowY/WS.zoomFactor - 100/(WS.zoomFactor+mag);
	if(WS.viewCM){	
		APP.context_top.drawImage(APP.imgM, xx,yy,200/(WS.zoomFactor+mag),200/(WS.zoomFactor+mag),x-200, y-200,200,200);
	}else{
		APP.context_top.drawImage(APP.imgC, xx,yy,200/(WS.zoomFactor+mag),200/(WS.zoomFactor+mag),x-200, y-200,200,200);	
	}
}
/***********************************************************************************************************************
 * 
...
Mammoapp.prototype.constructImpressionsForm = function(){
	...
	findingsHeaderDiv.innerHTML="You have found "+APP.numFoundAnswerFindings + 
		" out of "+ APP.answerfindings.length+str + "<br>Number or ROI's: "+APP.findings.length;
	...
	for(var j=0;j<APP.answerfindings.length;j++){
		...
		if(af.wasSubmitted){
			...
			//newDiv.style.width = '300px';
			//newDiv.style.height = 'automatic';
			newDiv.innerHTML = "<u>Your description</u>:<br>"+af.userDescription;
			fdiv.appendChild(newDiv);
			...
			//newDiv.style.width = '300px';
			//newDiv.style.height = 'automatic';
			newDiv.innerHTML = "<u>Answer description</u>:<br>"+af.description;
			fdiv.appendChild(newDiv);
		}
	}
	var newDiv = document.createElement('div');
	...
	if(APP.caseSubmitted){	
		...
		//List anything that was not found. May be made obsolete since we are not allowing
		//user to submit until all findings are found. Need input from docs.
		/*for(var i=0;i<APP.answerfindings.length;i++){
			if(APP.answerfindings[i].corFinding==-1){
				var newADiv = document.createElement('div');
				newADiv.className = 'answerFindingElement';
				newADiv.innerHTML = "Answer Finding "+(APP.answerfindings[i].findingNum+1)+"<br>"+
									"Type: "+APP.answerfindings[i].type+ "<br>"+
									"Description: " +APP.answerfindings[i].description;
		...
		//Update overall "score" (now irrelevant... will have to think of a better way to keep score):
...
Mammoapp.prototype.saveFinding = function(){
	APP.findings[APP.selectedFinding].description =document.getElementById("description_box").value;
	var mass =APP.check1.checked;
	var calc =APP.check2.checked;
	var other =APP.check3.checked;
	
	if(calc){APP.findings[APP.selectedFinding].type = "calc"; 
	}else if(mass){APP.findings[APP.selectedFinding].type = "mass";
	}else if(other){APP.findings[APP.selectedFinding].type = "other";}
}
