...
function callback_checkFindingAnswers(data){
	...
	for(var i=0;i<returnedAFs.length;i++){
		var newAF = new AnswerFinding(APP.imageNum,APP.setNum,returnedAFs[i].findingNum,
				...
				returnedAFs[i].type,returnedAFs[i].description,-1,"","",false);
	...
	/*for(var i=0;i<APP.findings.length;i++){
		APP.findings[i].corAnswerNums.length = 0;
		for(var j=0;j<APP.answerfindings.length;j++){		
			if(APP.findings[i].roiContainsAnswer(APP.answerfindings[j])){
				APP.findings[i].corAnswerNums.push(j);
				APP.answerfindings[j].corFinding = i;
			}			
		}
	}*/
