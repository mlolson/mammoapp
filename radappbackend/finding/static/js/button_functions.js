 ...
 *               
 *                Button functions: these functions interface 
 *                with the HTML button elements.
 *                
...
function but_gotoPrevFinding(){
	if(APP.findings.length>0){
		APP.selectedFinding--;
		if(APP.selectedFinding<0){APP.selectedFinding=APP.findings.length-1;}
		APP.drawScreen();
...
function but_gotoNextFinding(){
	if(APP.findings.length>0){
		APP.selectedFinding++;
		if(APP.selectedFinding>APP.findings.length-1){APP.selectedFinding = 0;}
		APP.drawScreen();
...
function but_checkFindingAnswers(){	
	...
	APP.User_Impression.description = document.getElementById("impressions_box").value;
	APP.User_Impression.indications = document.getElementById("indications_box").value;
	APP.User_Impression.finalPathology = document.getElementById("final_path_box").value;
...
function but_nextImage(){
	...
	if(APP.imageNum>APP.MaxImgNum){
		window.location = "/mammo/score/"+APP.TotalNumOfCorrectFindings+"/"+APP.TotalNumOfFindings+"/";	
		return;
	}
	APP.imgC.src = window.STATIC_URL+"media/set"+APP.setNum+"/"+(APP.imageNum)+"c.jpg";
    APP.imgM.src = window.STATIC_URL+"media/set"+APP.setNum+"/"+(APP.imageNum)+"m.jpg";
}
